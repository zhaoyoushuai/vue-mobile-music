...
export default {
  ...
  computed: {
    ...
    ...mapGetters(["fullScreen", "playlist", "currentSong","playing","currentIndex","mode",'sequenceList','playing'])
  },
  methods: {
    ...mapMutations({
    ...
    loop(){
      ...
      if(this.currentLyric && this.currentLyric.seck){
        console.log(this.currentLyric.seck)
        this.currentLyric.seck(0)
      }
    },
    //下一曲
    next(){
    ...
    middleTouchMove(e){
      ...
      this.$refs.lyricList.$el.style[transform] = `translate3d(${offsetWidth}px,0,0)` ;this.$refs.lyricList.$el.style[transitionDuration] = '0ms'
    ...
    middleTouchEnd(e){
      ...
      const time = 300
      this.$refs.lyricList.$el.style[transform] = `translate3d(${offsetWidth}px,0,0)`;  this.$refs.lyricList.$el.style[transitionDuration] = `${time}ms`
